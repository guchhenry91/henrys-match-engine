...
def frozen_losses(tmp_path, monkeypatch):
    ...
    for mid, ko in ((1, "2026-08-22T14:00:00Z"), (2, "2026-08-22T16:30:00Z")):
        picks.lock_pick(log, f"2026:{mid}", pick="Arsenal", confidence=5, kickoff=ko,
                        ...
                        board=True)          # frozen onto the board at lock time
...
def test_raising_the_threshold_does_not_evict_settled_picks(frozen_losses, monkeypatch):
    """Membership is frozen at lock time. Before this, raising the bar from 0.65 to
    0.70 silently deleted every past 0.66 pick -- winners and losers -- from the
    record, rewriting history the tier had already been judged on."""
    monkeypatch.setattr(publish.fixtures, "fetch_fixtures", lambda lg: frozen_losses)
    before = publish.build_best_picks()
    assert before["record"]["wrong"] == 2

    # The operator tightens the bar to 0.80, above the 0.72 these were locked at.
    # The two settled LOSSES must still count against the record.
    monkeypatch.setattr(publish, "BEST_PICK_MIN_PROB", 0.80)
    after = publish.build_best_picks()
    assert after["record"]["wrong"] == 2, (
        "raising the threshold erased settled losses from the record")
    assert len(after["settled"]) == 2


def test_a_pick_never_frozen_onto_the_board_stays_off_it(tmp_path, monkeypatch):
    """The other direction: LOWERING the bar must not retroactively promote picks
    that were never on the board, which would let past winners be adopted."""
    (tmp_path / "pl").mkdir()
    log = {}
    picks.lock_pick(log, "2026:1", pick="Arsenal", confidence=3,
                    kickoff="2026-08-22T14:00:00Z",
                    now=pd.Timestamp("2026-08-22T11:00:00Z"),
                    p_pick=0.55, board=False)          # below the bar at lock time
    picks.save_log(log, tmp_path / "pl" / "picks_log.json")
    fx = pd.DataFrame([{"match_id": 1, "round": 1,
                        "date": pd.Timestamp("2026-08-22T14:00:00"),
                        "home": "Arsenal", "away": "Chelsea", "played": True,
                        "home_goals": 3, "away_goals": 0}])   # it WON
    monkeypatch.setattr(publish, "PICKS_DIR", tmp_path)
    monkeypatch.setattr(publish, "OUT", tmp_path / "out")
    (tmp_path / "out").mkdir()
    monkeypatch.setattr(publish, "FILE_FOR", {"PL": "pl.json"})
